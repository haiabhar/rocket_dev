...
const rule_order_id = props.rule_order_id
...
const onSubmit = ({ value, touched }) => 
  { 
    ...
    updateData = () => {
      setLoading(true);
      fetch("api/create_rule_order",post_set)
  ...
  const fetchData = () =>
  {
    if(rule_order_id)
    {
      ...
      const config_settings = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json','X-CSRF-Token': csrf }, 
        body: JSON.stringify({"rule_order_id":rule_order_id})
      };
      setLoading(true);
      fetch("/api/get_rule_order",config_settings)
      .then((response) => response.json())
      .then((data) => {
        setFormData(data);
        setLoading(false);
    ...
    <Box gap="medium">
      ...
      <Header alignSelf="center" pad={{ horizontal: 'xxsmall' }}>
       ...
       <div direction="row">
        ...
        <TextInput id="rule_order_id" type="hidden" name="rule_order_id" value={rule_order_id}  /> 
        <div direction="row">
          <div className="col-md-12">
            <FormField  label="Priority Name" htmlFor="rule_order_name" >
              <TextInput id="rule_order_name" value={formData.name ? formData.name : ""} name="name" onChange={(e) => updateText(e)} />
            </FormField>
          </div>
      ...
      {open && (
        <Layer position="right" full={!['xsmall', 'small'].includes(size) ? 'vertical' : true} onEsc={onClose} >
          <Box fill="vertical" overflow="auto" width={!['xsmall', 'small'].includes(size) ? 'large' : undefined} pad="medium" >
            <LayerForm rule_order_id={props.rule_order_id} setData={props.setData} setOpen={value => setOpen(value)} />
          </Box>
        </Layer>
      )}
    </>
