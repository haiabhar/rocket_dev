...
const sub_category_id = props.sub_category_id
...
const onSubmit = ({ value, touched }) => 
  { 
    ...
    updateData = () => {
      setLoading(true);
      fetch("api/create_sub_category",post_set)
  ...
  const fetchData = () =>
  {
    if(sub_category_id)
    {
      ...
      setLoading(true);
      fetch("/api/get_sub_category",config_settings)
      .then((response) => response.json())
      .then((data) => {
        setFormData(data);
        setLoading(false);
    ...
    <Box gap="medium">
      ...
      <Header alignSelf="center" pad={{ horizontal: 'xxsmall' }}>
       ...
       <div direction="row">
        ...
        <TextInput id="sub_category_id" type="hidden" name="sub_category_id" value={sub_category_id}  /> 
        <div direction="row">
          <div className="col-md-12">
            <FormField  label="Sub-Category Name" htmlFor="sub_category_name" >
              <TextInput id="sub_category_name" value={formData.name ? formData.name : ""} name="name" onChange={(e) => updateText(e)} />
            </FormField>
          </div>
      ...
      {open && (
        <Layer position="right" full={!['xsmall', 'small'].includes(size) ? 'vertical' : true} onEsc={onClose} >
          <Box fill="vertical" overflow="auto" width={!['xsmall', 'small'].includes(size) ? 'large' : undefined} pad="medium" >
            <LayerForm category_id={props.category_id} sub_category_id={props.sub_category_id} setData={props.setData} setOpen={value => setOpen(value)} />
          </Box>
        </Layer>
      )}
    </>
