...
class Notification < ApplicationRecord
	...
	def self.trigger_after_action deed_obj

		status  = deed_obj.action_status
		rule_id  = deed_obj.rule_id
		if status == 'Success'
		 n_list	= self.where(rule_id: rule_id, sequence: ["After Action Success","After Action"])
		elsif status == 'Failed'
		 n_list	= self.where(rule_id: rule_id, sequence: ["After Action failure","After Action"])
		end
		if n_list.present?
		n_list.each do |n|
			notification = {}
		...
		# deed_obj.notification_sent = true
		# deed_obj.save
		end
	end
